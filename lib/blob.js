 ...
 *      read-after-write. Modul ini memasangnya di SETIAP cara membuat store,
 *      termasuk jalur auto-auth di platform: mode bawaan SDK adalah
 *      "eventual" yang bisa tertinggal sampai 60 detik, dan itu berarti
 *      pengguna yang baru login membaca sesi basi lalu terlempar "sesi
 *      berakhir", serta acknowledge yang baru ditulis terbaca lagi sebagai
 *      "pending". Bentuk `getStore(nama)` TIDAK punya tempat untuk opsi ini —
 *      jangan memakainya.
 *
...
function store() {
  ...
  if (projectId && token) {
    ...
    const { getStore } = require('@edgeone/pages-blob');
    // Bentuk OBJEK, bukan getStore(nama): bentuk satu-argumen tidak punya
    // tempat untuk `consistency`, dan SDK diam-diam memakai "eventual".
    // Itu berarti seluruh baca di produksi bisa tertinggal sampai 60 detik —
    // acknowledge yang baru ditulis terbaca lagi sebagai "pending".
    // projectId & token tetap datang dari platform (PAGES_PROJECT_ID +
    // kredensial yang ditambal builder); yang ditambahkan di sini hanya
    // mode konsistensinya.
 ...
 *
 * Teksnya sengaja menyebut langkah konkret: kegagalan ini hampir selalu berarti
 * variabel lingkungan belum terisi, dan pesan yang hanya bilang "tidak
 * terautentikasi" membuat orang mencari di tempat yang salah. Jangan menulis
 * bahwa kredensial Blob "otomatis di dalam Makers" — untuk Agent Functions yang
 * mendaftarkan SDK-nya di externalNodeModules, itu TIDAK benar.
 */
function explain(e) {
  const msg = String((e && e.message) || e);
  if (/credential|deployCredential|MISSING_PROJECT_ID|projectId|token/i.test(msg)) {
    const where = process.env.BLOB_LOCAL_DIR
      ? 'BLOB_LOCAL_DIR terisi, tetapi SDK tetap dipanggil — laporkan ini sebagai bug.'
      : 'Isi EDGEONE_PROJECT_ID dan EDGEONE_BLOB_TOKEN, lalu DEPLOY ULANG ' +
        '(perubahan variabel lingkungan baru berlaku setelah build baru). ' +
        'Token dibuat di konsol Makers -> Project Settings -> API Token. ' +
        'Cek hasilnya lewat /api/ping: "ready".blob harus true.';
    return new Error(`Blob belum punya kredensial. ${where} Penyebab asli: ${msg}`);
  }
  return e;
}
