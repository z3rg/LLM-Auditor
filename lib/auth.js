...
function validateRegistration({ name, email, password, divisionId }) {
  const errors = [];
  ...
  else if (!db.listDivisions().some((d) => d.id === Number(divisionId))) errors.push('Divisi tidak dikenal.');
  return errors;
}

// --- login throttle (in-memory, per email) ----------------------------------
const attempts = new Map(); // email -> { count, first }
...
function throttled(email) {
  ...
  if (!rec) return false;
  if (Date.now() - rec.first > WINDOW_MS) { attempts.delete(email); return false; }
  return rec.count >= MAX_ATTEMPTS;
}
function noteFailure(email) {
  const rec = attempts.get(email);
  if (!rec || Date.now() - rec.first > WINDOW_MS) attempts.set(email, { count: 1, first: Date.now() });
  else rec.count += 1;
}
function clearFailures(email) { attempts.delete(email); }

// --- sessions ---------------------------------------------------------------
function tokenHash(token) { return crypto.createHash('sha256').update(token).digest('hex'); }

function startSession(employeeId, userAgent) {
...
function currentUser(req) {
  ...
  const user = db.sessionUser(tokenHash(token), new Date().toISOString());
  if (!user) return null;
...
function sessionCookie(token, expires) {
  ...
  return `sid=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${maxAge}`;
}
...
function bootstrap() {
  ...
  const pending = db.staffWithoutPassword();
  if (!pending.length) return { seeded: [], seedPassword: null };
  const hash = hashPassword(seedPassword);
  for (const u of pending) db.setPassword(u.id, hash);
  db.purgeExpiredSessions(new Date().toISOString());
