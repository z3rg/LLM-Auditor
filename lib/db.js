...
function migrate() {
  db.exec(`
    ...
    -- Login sessions: opaque token (stored hashed) -> account.
  ...
  // Account columns on employees (added in place so quiz history, gaps and the
  // analytical views keep working for a registered participant).
  const cols = db.prepare('PRAGMA table_info(employees)').all().map((c) => c.name);
  if (!cols.includes('password_hash')) db.exec('ALTER TABLE employees ADD COLUMN password_hash TEXT');
  if (!cols.includes('status')) db.exec("ALTER TABLE employees ADD COLUMN status TEXT NOT NULL DEFAULT 'active'");
  if (!cols.includes('created_at')) db.exec('ALTER TABLE employees ADD COLUMN created_at TEXT');
  // One account per email address (case-insensitive).
  try { db.exec('CREATE UNIQUE INDEX IF NOT EXISTS idx_employees_email ON employees(email COLLATE NOCASE)'); }
  catch (_) { /* pre-existing duplicate emails: leave the index off */ }
...
function createUser({ name, email, passwordHash, divisionId, role = 'employee' }) {
  const id = Number(db.prepare(`
    INSERT INTO employees (name, email, role, division_id, password_hash, status, created_at)
    VALUES (?, ?, ?, ?, ?, 'active', ?)
  `).run(name, email, role, divisionId, passwordHash, new Date().toISOString()).lastInsertRowid);
...
/** Seeded staff accounts that cannot log in yet (no password set). */
function staffWithoutPassword() {
  return db.prepare(`
...
/** Every account that can sign in, plus the seeded staff placeholders. */
function listAccounts() {
  return db.prepare(`
    SELECT ${ACCOUNT_FIELDS},
           (e.password_hash IS NOT NULL AND e.password_hash <> '') AS registered,
           (SELECT COUNT(*) FROM sessions s WHERE s.employee_id = e.id AND s.expires_at > ?) AS active_sessions
    FROM employees e JOIN divisions d ON d.id = e.division_id
    WHERE e.password_hash IS NOT NULL AND e.password_hash <> ''
    ORDER BY e.role <> 'super_admin', e.created_at IS NULL, e.created_at DESC, e.name
  `).all(new Date().toISOString());
}
function setUserRole(id, role) {
  db.prepare('UPDATE employees SET role = ? WHERE id = ?').run(role, id);
  return accountById(id);
}
function setUserStatus(id, status) {
  db.prepare('UPDATE employees SET status = ? WHERE id = ?').run(status, id);
  if (status !== 'active') deleteSessionsForUser(id);
  return accountById(id);
}

function createSession({ tokenHash, employeeId, createdAt, expiresAt, userAgent }) {
  db.prepare(`
    INSERT INTO sessions (token_hash, employee_id, created_at, expires_at, user_agent)
    VALUES (?, ?, ?, ?, ?)
  `).run(tokenHash, employeeId, createdAt, expiresAt, userAgent || '');
}
function sessionUser(tokenHash, nowIso) {
  return db.prepare(`
    SELECT ${ACCOUNT_FIELDS}
    FROM sessions s
...
function deleteSessionsForUser(employeeId) {
