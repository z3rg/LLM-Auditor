...
async function reseed() {
  ...
  await blob.del(K.seedLock);
  blob.invalidate();
  return seed();
}

/**
 * Isi data awal bila store masih kosong. Idempoten dan aman dipanggil
 * bersamaan oleh beberapa instance.
 *
 * Klaim dilakukan lewat tulis bersyarat pada kunci kunci-penanda SEBELUM data
 * ditulis: tanpa itu dua request dingin yang datang bersamaan sama-sama lolos
 * pemeriksaan "sudah terisi?" lalu menulis dua kali. onlyIfNew adalah satu-
 * satunya operasi atomik yang punya Blob, jadi itu yang dipakai.
 */
async function seed() {
  if (await blob.getJSON(K.seed, null)) return null; // sudah terisi
  if (!(await blob.createJSON(K.seedLock, { at: new Date().toISOString() }))) {
    return null; // instance lain sedang mengisi
  }
