  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (c === '}' && stack.length) {
      ...
      try {
        ...
        if (obj && typeof obj.question === 'string' && Array.isArray(obj.options)) out.push(obj);
...
${QUIZ_SCHEMA_HINT}`,
  ...
  const out = await chat(messages, { temperature: 0.5, json: true, max_tokens: 8000 });
...
Tambahan WAJIB: tiap objek soal punya "block": <nomor blok 1..${n}> sesuai blok yang menjadi dasarnya.`,
  ...
  const out = await chat(messages, { temperature: 0.45, json: true, max_tokens: 8000 });
  const parsed = safeJson(out.content) || {};
  return {
...
async function generateQuizPlanned(topic, area, n = 10) {
  ...
  if (gen.finishReason) trace.push({ step: 'generate', finishReason: gen.finishReason, rawLength: gen.rawLength, valid: questions.length });
